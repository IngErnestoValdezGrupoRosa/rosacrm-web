...
async function syncDealsToSupabase() {
  if (!_supabase || _isSyncingDeals) return;
  _isSyncingDeals = true;
  try {
    for (const d of STATE.deals) {
      // Asegurar que el deal tenga un UUID válido
      if (!isUUID(d.id)) {
        d.id = generateUUID();
      }
      const row = {
...
function setupAuth() {
  ...
  _supabase.auth.onAuthStateChange((event, session) => {
    ...
    if (session && session.user) {
      ...
      document.getElementById('sidebar-user-name').textContent = session.user.email.split('@')[0];
      document.getElementById('sidebar-user-avatar').textContent = session.user.email.substring(0, 2).toUpperCase();
      ...
      if (splashScreen) splashScreen.style.display = 'none'; // Quitar splash rápido
    }
  });
  ...
  document.getElementById('auth-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        sucDiv.textContent = "Registro exitoso. Si requieres confirmación, revisa tu correo.";
